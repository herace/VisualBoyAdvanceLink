...
struct option sdlOptions[] = {
  ...
  { "linkaddr", required_argument, 0, 'M' },
  { "linktime", required_argument, 0, 'Q' },
  { "linknum", required_argument, 0, 'P' },
...
Options:\n\
  ...
  --link int                   Set Multiplayer Link\n\
                                0 - Be the Server\n\
                                1 - Be the Client\n\
  --linkaddr IP                Set IP Server Address (Option for Client)\n\
  --linktime int               Set Time Out (ms) (Option for Server and Client)\n\
  --linknum int                Set Number of Players [2-4] (Option for Server)\n\n\
  ...
  if(optind < argc) {
	...
	if(linkmode==1)
		...
		if(lanlink.server==1)
			...
			lanlink.speed=0;
			lspeed=lanlink.speed;
			lc.Init(gethostbyname(ipserver), NULL);
			}
		}
